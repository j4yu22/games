# ronso(test) link
# https://docs.google.com/spreadsheets/d/1k84-qa4sIKYmHdHqOOWpVCJlJ4DFXfSwwgnZu2mZ03g/edit?usp=sharing
# general Gsheet 2.1 link
# https://docs.google.com/spreadsheets/d/1ApmbXHTln99fPTUpanyQRTXNzXbQ8UBTt3Uq8xInQKw/edit
...
def get_batch_cell_values(service, spreadsheet_id, ranges):
    """
    Fetches the values from the specified cell ranges in the Google Sheet.

    Parameters:
    service (Resource): The Sheets API service instance.
    spreadsheet_id (str): The ID of the spreadsheet.
    ranges (list): The list of cell ranges to fetch the values from (e.g., ["v2.1!A1", "v2.1!A1:B2"]).
    ...
    for value_range in value_ranges:
        ...
        if not values:
            ...
            # Flatten the list of lists and concatenate values with a space
            flattened_values = ' '.join([str(item) for sublist in values for item in sublist])
            # Clean the data by removing special characters (like circles)
            cleaned_values = re.sub(r'[^\x00-\x7F]+', '', flattened_values)  # Removes non-ASCII characters
            cleaned_values = re.sub(r'\s+', ' ', cleaned_values).strip()  # Replace multiple spaces with a single space
...
def parse_character_sheet(service, spreadsheet_id):
    """
    Parses the character sheet data from the Google Sheets document.

    Parameters:
        service (Resource): The Google Sheets API service instance.
    ...
    # Define cell addresses for different attributes
    cells_to_fetch = [
    ...
    values = get_batch_cell_values(service, spreadsheet_id, cells_to_fetch)

    # Map values to data dictionary
    data = {
...
def main():
    ...
    character_data = parse_character_sheet(service, spreadsheet_id)
